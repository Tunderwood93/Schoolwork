...
static void Interpret(int start)
{
  ...
  while (cont) {
    ...
    switch (opcode) {
      case 0x00:
        switch (funct) {
          case 0x00: /* sll */reg[rd] = reg[rs] << shamt; break; // added 
          case 0x03: /* sra */ reg[rd] = reg[rs] >> shamt; break; // added
          case 0x08: /* jr */ pc = reg[rs]; break; // added
          case 0x10: /* mfhi */ reg[rd] = hi; break; // added reg[rd]=hi; 
          ...
          case 0x21: /* addu */ reg[rd] = reg[rs] + reg[rt]; break; // added reg[rd] = reg[rs] + reg[rt];
          case 0x23: /* subu */ reg[rd] = reg[rs] - reg[rt]; break;
          case 0x2a: /* slt */ reg[rd] = (reg[rs] < reg[rt]) ? 1 : 0; break;
      ...
      case 0x04: /* beq */ if(reg[rs] == reg[rt]){pc = pc + simm * 4;} break; // Could be pc = (reg[rs] == reg[rt]) ? pc + simm * 4
      case 0x05: /* bne */ if(reg[rs] != reg[rt]){pc = pc + simm * 4;}break; // Could be pc = (reg[rs] != reg[rt]) ? pc + simm * 4
      case 0x09: /* addiu */ reg[rt] = reg[rs] + simm; break;
      case 0x0c: /* andi */ reg[rt] = reg[rs] & uimm; break;
      case 0x0f: /* lui */ reg[rt] = simm << 16; break; //Found on MIPS Wiki lui can use simm or uimm 
