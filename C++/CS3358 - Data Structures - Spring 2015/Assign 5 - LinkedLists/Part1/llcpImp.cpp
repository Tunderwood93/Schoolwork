...
bool DelstartTargetNode(Node*& headPtr, int target)
{
   Node *precursor = 0,
        *cursor = headPtr;
...
void MakeDistinctMirrorPairs(Node*& head)
{
   ...
   while (start != 0)
   {
      while(cursor != 0)
      {
         if (start->data == cursor->data && start->link == cursor)
         {
            found = true;
            prev = cursor;
            cursor = cursor->link;
         ...
         if (start->data == cursor->data && found == false)
         {
            found = true;
         ...
         if (start->data == cursor->data && found == true)
         {
            prev->link = cursor->link;
            delete cursor;
            cursor = prev->link;
